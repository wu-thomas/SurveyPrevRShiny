...
if(FALSE){
  ...
  #file_path <-'E:/Downloads/KE_2022_DHS_06192024_2037_143411.zip'
  #file_path <-'C:/Users/wu-th/Downloads/RW_2019-20_DHS_04082024_724_143411.zip'
  #file_path <-'C:/Users/wu-th/Downloads/KE_2022_DHS_06142024_1551_143411.zip'
  ...
  if(!CountryInfo$WHO_version()){
    country_shapefile <- get_country_shapefile(country=ex.country,source='GADM')
  }else{
  ...
  ### test wrong clusters
  adm1.cluster.info <- surveyPrev::clusterInfo(geo=GPS.dat,
                                          poly.adm1=CountryInfo$GADM_list()[[paste0('Admin-',1)]],
                                          poly.adm2=CountryInfo$GADM_list()[[paste0('Admin-',1)]],
  ...
  res_adm <- surveyPrev::directEST(data=CountryInfo$svy_analysis_dat(),
                                   cluster.info= adm1.cluster.info,
                                   admin=0,
                                   strata="all",
                                   alt.strata=NULL)
  ...
  examine.res$Direct$National$res.admin0
...
###############################################################
### test Dominican Republic contraceptive
###############################################################
...
if(FALSE){
  ...
  ### country meta
  ex.country <- 'Dominican Republic'
  ex.svy.year <- '2013'
  strat.gadm.level <- 1

  ### indicator
  ex.indicator.abbrev <-'FP_CUSA_W_MOD'
  #file_path <-'E:/Downloads/DR_2013_DHS_05292024_73_143411.zip'
  ...
  tmp.scatter <- scatter.plot( res.obj.x = examine.res$Direct$`Admin-3`,
                               res.obj.y = examine.res$Unit$`Admin-3`,
                               value.to.plot = 'mean',
                               model.gadm.level = 3,
                               strata.gadm.level = 1,
                               label.x = 'M1',
...
if(FALSE){
  ...
  ### country meta
  ex.country <- 'United Republic of Tanzania'
  ex.svy.year <- '2022'
  strat.gadm.level <- 1

  ### indicator
  ex.indicator.abbrev <-'CN_NUTS_C_HA2'
  #file_path <-'E:/Downloads/TZ_2022_DHS_08152024_1446_143411.zip'
  #file_path <-'C:/Users/wu-th/Downloads/RW_2019-20_DHS_04082024_724_143411.zip'
  #file_path <-'C:/Users/wu-th/Downloads/RW_2019-20_dat.zip'
  ...
  direct_adm2_res <- examine.res$Direct$`Admin-2`$res.admin2
  cluster.info.adm2 <- geo_info_list$`Admin-2`$cluster.info$data
  modt<- dplyr::left_join(analysis_dat,cluster.info.adm2,by="cluster")

  modt <- modt[!(is.na(modt$value)|is.na(modt$admin2.name.full)),]

  Ukerewe_modt <- modt[modt$admin2.name.full=='Mwanza_Ukerewe',]
  ...
  tmp_res = smoothSurvey_res$HT

  poly.adm0 <- country_shapefile$country_shp_analysis$National
  poly.adm1 <- country_shapefile$country_shp_analysis$`Admin-1`
  poly.adm2 <- country_shapefile$country_shp_analysis$`Admin-2`

  raw.dat <- recode.data
  raw.stunt.data <- analysis_dat

  analysis.dat <- modt
  GPS.dat <- GPS.dat

  save(poly.adm0,poly.adm1,poly.adm2,raw.dat,raw.stunt.data,analysis.dat,GPS.dat,file="TZA_stunt_data.rda")
library(dplyr)
  tab_res = analysis.dat %>%
    group_by(admin2.name.full) %>%
    mutate(unique_types = n_distinct(cluster))  %>%
    distinct(admin2.name.full, .keep_all = T)

  table(tab_res$unique_types)
...
if(FALSE){
  ...
  ### indicator
  ex.indicator.abbrev <-'WS_SRCE_P_BAS'
  #file_path <-'E:/Downloads/GH_2022_DHS_09042024_1925_143411.zip'
  #file_path <-'E:/Downloads/GH_2014_DHS_09042024_1934_143411.zip'
  #file_path <-'C:/Users/wu-th/Downloads/RW_2019-20_dat.zip'
  ...
  #sum(is.na(is.na(ph_wtr_source)))
  table(svy_dat_recode$hv205)


  svy_dat_recode$hv205[1]
